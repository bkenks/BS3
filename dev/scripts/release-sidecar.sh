#!/usr/bin/env bash
# Release the BS3 SIDECAR: build the multi-platform image, push it to the GitHub
...
#   ./dev/scripts/release-sidecar.sh <version> [--prerelease]
#
...
REGISTRY="ghcr.io"
OWNER="bkenks"
IMAGE_NAME="bs3-sidecar"
PLATFORMS="linux/amd64,linux/arm64"
# ─────────────────────────────────────────────────────────────

usage() {
  echo "Usage: $0 <version> [--prerelease]" >&2
  echo "  <version>  bare semver X.Y.Z (e.g. 0.1.0)" >&2
  exit 1
}
...
# Build context must be the repo root (Dockerfile.sidecar pulls in cli-tool/ and
# the sibling logger/ module). This script lives in dev/scripts/, so the repo
...
# reused on every later run (shared with the server release).
...
# ── GitHub release ───────────────────────────────────────────
TAG="sidecar/v$VERSION"
TARGET="$(git rev-parse HEAD)"

if [ "$PRERELEASE" = false ]; then
  NOTES="BS3 Sidecar v$VERSION
...
Platforms: $PLATFORMS"
  ...
  NOTES="BS3 Sidecar v$VERSION (prerelease)
...
# `gh release create` creates the sidecar/v$VERSION tag itself.
if [ "$PRERELEASE" = true ]; then
  gh release create "$TAG" \
    --title "Sidecar v$VERSION" \
    --target "$TARGET" \
    --notes "$NOTES" \
    --prerelease
else
...
echo "──────────────────────────────────────────────────────────"
echo "Sidecar release done: image $IMAGE:$VERSION pushed, GitHub release $RELEASE_URL"
